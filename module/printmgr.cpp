...
bool printmgr::printReport(QPainter &painter){
    ...
    painter.drawText(TitleRect,Qt::AlignCenter,panelName+tr("报告"));
    ...
    if (refValid)
        ...
        painter.drawText(InnerRefRect,Qt::AlignVCenter,tr("内部参考品：")+tr("无效 x"));
    painter.drawText(PanelNameRect,Qt::AlignVCenter,panelName);
    painter.drawText(SerialRect,Qt::AlignVCenter,tr("批号：")+ExGlobal::boxSerial());
    painter.drawText(ValidDateRect,Qt::AlignVCenter,tr("有效期：")+validTime.split(" ")[0]);
    ...
    painter.drawText(ItemNameRect,Qt::AlignVCenter,tr("病菌"));
