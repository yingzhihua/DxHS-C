...
/*
    res[2] = 1;
    res[3] = 1;
    strQr = "FLASHDX1 80004 301 GM01 2021032310 365 X";
    res.append(strQr);
    emit finishQRcode(res);
    return;
*/

    bool bCKCamera = false;
    if (!OpenCamera(1,cap)){
        Log::LogCam(QString("Failed to open camera1"));
        qDebug()<<"Failed to open camera1";
        if (!OpenCamera(0,cap)){
            Log::LogCam(QString("Failed to open camera0"));
            qDebug()<<"Failed to open camera0";
            if (!CKCameraDrv::openCamera(true)){
                qDebug()<<"Failed to open CKCamera";
                res.append("Cannot identify");
                emit finishQRcode(res);
                return;
            }
            bCKCamera = true;
            qDebug()<<"qrcoder open CKCamera";
        }
        else
            qDebug()<<"qrcoder open camera0";
    }
    else
        qDebug()<<"qrcoder open camera1";

    if (!bCKCamera){
        cap.set(CAP_PROP_FRAME_WIDTH,1920);
        cap.set(CAP_PROP_FRAME_HEIGHT,1080);
        cap.set(CAP_PROP_AUTO_EXPOSURE,1);
        cap.set(CAP_PROP_EXPOSURE,ExGlobal::poxValue);
        qDebug()<<"exposure="<<ExGlobal::poxValue<<"width:"<<cap.get(CAP_PROP_FRAME_WIDTH)<<" height:"<<cap.get(CAP_PROP_FRAME_HEIGHT)<<" exp:"<<cap.get(CAP_PROP_EXPOSURE);
    }
    else{
        CKCameraDrv::setResolution(15,true);
        CKCameraDrv::setabsExpose(ExGlobal::poxValue,true);
        qDebug()<<"exposure="<<ExGlobal::poxValue;
    }

    strQr.clear();
    for (;;) {               
        Mat handleFrame = Mat(1080,1920,CV_8UC1,ExGlobal::hbufrgb);
        if (bCKCamera){
            uint32_t len = CKCameraDrv::GetFrame(ExGlobal::bufrgb,true);
            memcpy(ExGlobal::hbufrgb,ExGlobal::bufrgb,1080*1920);
            if (len == 0)
                break;
            img = QImage(ExGlobal::bufrgb,handleFrame.cols,handleFrame.rows,QImage::Format_Grayscale8);
        }
        else{
            Mat Frame;
            cap>>Frame;
            ...
            img = QImage(ExGlobal::bufrgb,sourceFrame.cols,sourceFrame.rows,QImage::Format_RGB888);
        }

        qDebug()<<"read Frame "<<count<<",time:"<<QDateTime::currentDateTime().toString("hh:mm:ss.zzz")<<",cols="<<handleFrame.cols<<",rows="<<handleFrame.rows<<"scale:"<<scale;
        Log::LogCam(QString("read Frame %1,time:%2,cols=%3,rows=%4,type=%5").arg(count).arg(QDateTime::currentDateTime().toString("hh:mm:ss.zzz")).arg(handleFrame.cols).arg(handleFrame.rows).arg(handleFrame.type()));
        ...
        {
            Mat sourceFrame = handleFrame(Rect(initX,initY,QUIDS_WIDTH,QUIDS_HEIGHT)).clone();
            if (haveliquids(sourceFrame)){
                qDebug()<<"haveliquids";
                res[3] = 1;
            }
            else
            ...
            Mat liquids = handleFrame(Rect(initX,initY,QUIDS_WIDTH,QUIDS_HEIGHT)).clone();
            img2 = QImage(liquids.data,liquids.cols,liquids.rows,QImage::Format_Grayscale8);
            img3 = QImage(sourceFrame.data,sourceFrame.cols,sourceFrame.rows,QImage::Format_Grayscale8);
        }        
        break;
    }
    if (bCKCamera)
        CKCameraDrv::closeCamera(true);
    else
        cap.release();
...
//*/
    ...
    morphologyEx(image,image,MORPH_OPEN,element); //先腐蚀后膨胀
    blur(image,image,Size(3,3));

    zbar::ImageScanner scanner;
    scanner.set_config(zbar::ZBAR_NONE,zbar::ZBAR_CFG_ENABLE,1);

    zbar::Image imageZbar(static_cast<uint>(image.cols),static_cast<uint>(image.rows),"Y800",image.data,static_cast<uint64>(image.cols*image.rows));
    scanner.scan(imageZbar);    
    if (imageZbar.symbol_begin()==imageZbar.symbol_end()){
        qDebug()<<"zbar first scan fail";
        Log::LogCam("zbar first scan fail");
        adaptiveThreshold(image,image,255,ADAPTIVE_THRESH_GAUSSIAN_C,THRESH_BINARY,71,0);
        scanner.scan(imageZbar);
    }
    ...
    if (imageZbar.symbol_begin()==imageZbar.symbol_end()){
        ...
        {            
            qDebug()<<"point "<<symbol->get_location_x(static_cast<uint>(i))<<symbol->get_location_y(static_cast<uint>(i));
        }
...
void QRcoder::DebugQr(){
    QString debugFile = QCoreApplication::applicationDirPath()+"/debug/Qr.bmp";
    Mat Frame  = cv::imread(debugFile.toStdString());
    if (Frame.empty())
        return;
    Mat handleFrame = Mat(Frame.rows,Frame.cols,CV_8UC1,ExGlobal::hbufrgb);
    cvtColor(Frame, handleFrame, COLOR_RGB2GRAY);
    blur(handleFrame,handleFrame,Size(3,3));
    Mat element = getStructuringElement(MORPH_ELLIPSE,Size(5,5));
    morphologyEx(handleFrame,handleFrame,MORPH_OPEN,element); //先腐蚀后膨胀
    blur(handleFrame,handleFrame,Size(3,3));
    saveImage(handleFrame,"Debug",true);

    QRCodeDetector qrcode;
    vector<Point> transform;
    bool result_detection = qrcode.detect(handleFrame,transform);
    qDebug()<<"result_detection"<<result_detection;

    zbar::ImageScanner scanner;
    scanner.set_config(zbar::ZBAR_NONE,zbar::ZBAR_CFG_ENABLE,1);
    qDebug()<<"DebugQr"<<Frame.type()<<Frame.channels();
    zbar::Image imageZbar(handleFrame.cols,handleFrame.rows,"Y800",handleFrame.data,handleFrame.cols*handleFrame.rows);
    scanner.scan(imageZbar);
    if (imageZbar.symbol_begin()==imageZbar.symbol_end()){
        qDebug()<<"zbar first scan fail";
        adaptiveThreshold(handleFrame,handleFrame,255,ADAPTIVE_THRESH_GAUSSIAN_C,THRESH_BINARY,71,0);
        scanner.scan(imageZbar);
    }
    if (imageZbar.symbol_begin()==imageZbar.symbol_end()){
        qDebug()<<"zbar scan fail";
        ...
        zbar::Image::SymbolIterator symbol = imageZbar.symbol_begin();
        strQr = QString::fromStdString(imageZbar.symbol_begin()->get_data());
        qDebug()<<"decode_info:"<<strQr<<"point size"<<symbol->get_location_size()<<symbol->get_location_x(3)<<symbol->get_location_y(3);
    }
}
