...
int ImageAnalysis::FocusX = 1000;
int ImageAnalysis::FocusY = 670;
int ImageAnalysis::FocusWidth = 600;
int ImageAnalysis::FocusHeight = 600;
int ImageAnalysis::LightCX = 1300;
int ImageAnalysis::LightCY = 1000;
int ImageAnalysis::LightR = 300;

static int scapx,scapy,scapwidth,scapheight;
...
ImageAnalysis::ImageAnalysis(QObject *parent) : QObject(parent)
{
    ...
    blackImg = Mat::zeros(imageHeight,imageWidth,CV_16U);
}

void ImageAnalysis::FindShift(Mat img, int mpp, Point &point){
    vector<int> addx,addy,adx;
    QHash<size_t,int> samecount;
    Mat scopeimg;
    img(Rect(scapx,scapy,scapwidth,scapheight)).copyTo(scopeimg);
    FindGrid(scopeimg,mpp,addx,addy);

    for (size_t i = 0; i < addx.size()&&i < offsetx.size(); i++)
    {
        int dx = addx[i] - offsetx[i];
        for (size_t j = 0; j < offsetx.size(); j++){
            if (abs(dx)>abs(addx[i]-offsetx[j]))
                dx = addx[i]-offsetx[j];
        }        
        adx.push_back(dx);
    }
    ...
    size_t select = 0;
    foreach(size_t index, samecount.keys()){
        if (samecount[index] > samecount[select])
            select = index;
        else if(samecount[index] == samecount[select]){
            if (abs(adx[index]) < abs(adx[select]))
                select = index;
        }
    }
    point.x = firstCenter.x + adx[select];

    adx.clear();
    samecount.clear();
    for (size_t i = 0; i < addy.size() && i < offsety.size(); i++)
    {
        int dy = addy[i] - offsety[i];
        for (size_t j = 0; j < offsety.size(); j++){
            if (abs(dy)>abs(addy[i]-offsety[j]))
                dy = addy[i]-offsety[j];
        }
        adx.push_back(dy);
    }
    ...
    point.y = firstCenter.y + adx[select];
}
...
QImage ImageAnalysis::getMainImg(int type,int light){
    ...
    if (type == 0){
        line(tempImg,Point(scapx,scapy),Point(scapx+scapwidth,scapy),Scalar(255),20);
        line(tempImg,Point(scapx,scapy+scapheight),Point(scapx+scapwidth,scapy+scapheight),Scalar(255),20);
        line(tempImg,Point(scapx,scapy),Point(scapx,scapy+scapheight),Scalar(255),20);
        line(tempImg,Point(scapx+scapwidth,scapy),Point(scapx+scapwidth,scapy+scapheight),Scalar(255),20);
...
void ImageAnalysis::MaskHandle(void *data, int imageType, QString fileName){
    Mat temp;
    imageCount = 0;
    x.clear();
    y.clear();

    qDebug()<<"MaskHandle:"<<imageType;
    if (imageType == 0)
    {
    ...
    FindGrid(temp,mpp,x,y);
    qDebug()<<"FindGrid:"<<x.size()<<","<<y.size();
    ...
    //crop
    scapy = y[static_cast<uint64>(topPointy)] - subsize;
    if (scapy < 0)
        scapy = 0;
    int bottom = y[static_cast<uint64>(topPointy+gridCols-1)] + subsize;
    if (bottom >= imageHeight)
        bottom = imageHeight - 1;
    scapx = x[static_cast<uint64>(topPointx)] - subsize;
    if (scapx < 0)
        scapx = 0;
    int right = x[static_cast<uint64>(topPointx+gridRows-1)] + subsize;
    if (right >= imageWidth)
        right = imageWidth - 1;
    scapwidth = right - scapx + 1;
    scapheight = bottom - scapy + 1;
    offsetx.clear();
    offsety.clear();
    FindGrid(temp(Rect(scapx,scapy,scapwidth,scapheight)),mpp,offsetx,offsety);
    imwrite(fileName.toStdString(),temp(Rect(scapx,scapy,scapwidth,scapheight)));
}

void ImageAnalysis::AddImage(void *data, int imageType, QString fileName){
    Mat temp;
    if (x.size()<gridCols || y.size()<gridRows)
        return;
    imageCount++;
...
void ImageAnalysis::SetBlackImage(void *data, int imageType){
    if (imageType == 1)
        Mat(imageHeight,imageWidth,CV_16U,data).copyTo(blackImg);
}
...
int ImageAnalysis::subImageHandle(bool debug, size_t posX, size_t posY, Mat &img, Mat &subimg,vector<vector<Point>> &contours,Point &centerPoint){
    ...
    //imclose
    Mat element = getStructuringElement(MORPH_ELLIPSE,Size(9,9));
    morphologyEx(subimg,subimg,MORPH_OPEN,element); //先腐蚀后膨胀
    morphologyEx(subimg,subimg,MORPH_CLOSE,element);
    //morphologyEx(subimg,subimg,MORPH_CLOSE,element);
    ...
    for (size_t z = 0; z < contours.size(); z++)
    {
        ...
        if (valid){
            ...
            if (s>800 && s<8000 && (4*3.14*s/l/l)>0.3)
            {
                if (center == -1)
                {
...
void ImageAnalysis::FindGrid(Mat img, int mpp, vector<int> &x, vector<int> &y){
    ...
    //mean
    GaussianBlur(img, img, Size(25,25),0);
    Mat_<ushort>::iterator it = img.begin<ushort>();
    for (int i = 0; i < h; i++)
    {
        ynumber[i] = 0;
        for(int j = 0; j < w; j++)
...
int ImageAnalysis::SpotCal(int PosX, int PosY, Mat &img, Point& offset, QVector<int> &value){
    int topsub = y[static_cast<uint64>(PosY+topPointy)] - (subsize>>1);
    int bottom = y[static_cast<uint64>(PosY+topPointy)] + (subsize>>1);
    int leftsub = x[static_cast<uint64>(PosX+topPointx)] - (subsize>>1);
    int rightsub = x[static_cast<uint64>(PosX+topPointx)] + (subsize>>1);

    if (PosY+topPointy == static_cast<int>(debugy) && PosX+topPointx == static_cast<int>(debugx))
...
double ImageAnalysis::GetDefinition(void *data, int imageType){
    ...
    Mat imageGrey;
    if (imageType == 0){
        Mat(imageHeight,imageWidth,CV_8U,data)(Rect(FocusX,FocusY,FocusWidth,FocusHeight)).copyTo(imageGrey);
        Sobel(imageGrey,imageSobel,CV_8U,1,1);
    }
    else if(imageType == 1){
        Mat(imageHeight,imageWidth,CV_16U,data)(Rect(FocusX,FocusY,FocusWidth,FocusHeight)).copyTo(imageGrey);
...
double ImageAnalysis::GetMeanLight(void *data, int imageType){
    ...
    if (imageType == 0){
        ...
        Mat(imageHeight,imageWidth,CV_16U,data)(Rect(imageHeight/4,imageWidth/4,imageHeight/2,imageWidth/2)).copyTo(imageGrey);
    }
...
double ImageAnalysis::GetLLight(void *data, int imageType){
    double meanValue = 0.0;
    Mat imageGrey;
    Mat maskImage = Mat::zeros(LightR<<1,LightR<<1,CV_8U);
    circle(maskImage,Point(LightR,LightR),LightR,Scalar(1),-1);

    if (imageType == 0){
        Mat(imageHeight,imageWidth,CV_8U,data)(Rect(LightCX-LightR,LightCY-LightR,LightR*2,LightR*2)).copyTo(imageGrey);
    }
    else if(imageType == 1){
        Mat(imageHeight,imageWidth,CV_16U,data)(Rect(LightCX-LightR,LightCY-LightR,LightR*2,LightR*2)).copyTo(imageGrey);
    }
    meanValue = mean(imageGrey,maskImage)[0];

    qDebug()<<"GetLLight,elapsed="<<LightCX<<LightCY<<LightR<<maskImage.cols<<imageGrey.cols;
    return meanValue;
}
