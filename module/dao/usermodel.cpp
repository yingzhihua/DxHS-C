...
QStringList UserModel::getLogUser(){
    ...
    QSqlQuery query = SqliteMgr::select("select tParam1,Logid from (select tParam1,Logid from Log where LogType = 2 and iParam1 = 0 order by Logid desc) tt group by lower(tParam1)");
    while(query.next())
    {
        if (query.value(0).toString() != "flashdx"){
