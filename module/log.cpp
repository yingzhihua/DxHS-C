...
QString Log::readSerial(){
    QFile file("/opt/serial.txt");
...
bool Log::writeSerial(QString serial){
    qDebug()<<"writeSerial"<<serial;
    QFile file;
    file.setFileName("/opt/serial.txt");
    if (!file.open(QIODevice::WriteOnly|QIODevice::Text))
    {
        qDebug()<<"Critical file open error!";
    }
    QTextStream textStream(&file);
    textStream<<serial<<endl;
    textStream.flush();
    file.close();
    return true;
}
